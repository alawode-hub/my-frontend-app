      ...
      <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "4rem 2rem" }}>
        ...
        {confirmAction && (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
            <div style={{ background: '#111', border: '2px solid #fff', padding: '30px', maxWidth: '400px', textAlign: 'center' }}>
              <p style={{ fontSize: '1.1rem', fontWeight: '700', marginBottom: '25px', textTransform: 'uppercase' }}>{confirmAction.text}</p>
        ...
        {cartItems.length === 0 ? (
          <div style={{ textAlign: "center", padding: "4rem 0" }}>
            ...
            <Link to="/shop" style={{ background: "#FF0000", color: "#fff", padding: "1rem 2rem", textDecoration: "none", fontWeight: "700", letterSpacing: "1px" }}>CONTINUE SHOPPING</Link>
          </div>
        ) : (
          <div className="cart-layout">
            
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "3fr 1fr 1fr", paddingBottom: "1rem", borderBottom: "1px solid #222", marginBottom: "2rem" }}>
                  ...
                  return (
                    <div key={item._id} className="cart-item-grid">
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem" }}>
                        <button onClick={() => handleUpdateQty(item._id, qty - 1)} disabled={qty <= 1} style={{ background: "transparent", border: "none", color: "#fff", fontSize: "1.5rem", cursor: qty <= 1 ? "not-allowed" : "pointer", padding: "0 0.5rem" }}>-</button>
                        <span style={{ fontSize: "1rem", fontWeight: "600", minWidth: "20px", textAlign: "center" }}>{qty}</span>
                        <button onClick={() => handleUpdateQty(item._id, qty + 1)} disabled={qty >= stock} style={{ background: "transparent", border: "none", color: "#fff", fontSize: "1.5rem", cursor: qty >= stock ? "not-allowed" : "pointer", padding: "0 0.5rem" }}>+</button>
                      </div>

                      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: "1rem" }}>
                        <span style={{ fontSize: "1rem", fontWeight: "700" }}>₦{safeItemTotal.toLocaleString()}</span>
                        <button onClick={() => handleRemove(item._id)} style={{ background: "transparent", border: "none", color: "#666", cursor: "pointer", padding: "8px" }}>
              ...
              <button onClick={handleClearCart} style={{ background: "transparent", border: "none", color: "#FF0000", fontSize: "0.75rem", fontWeight: "700", letterSpacing: "1px", cursor: "pointer", marginTop: "2rem" }}>CLEAR CART</button>
            </div>

            <div style={{ background: "#111", padding: "2rem", position: "sticky", top: "2rem" }}>
              <h2 style={{ fontSize: "1rem", fontWeight: "700", letterSpacing: "1.5px", marginBottom: "2rem" }}>ORDER SUMMARY</h2>
              ...
              <div style={{ borderTop: "1px solid #222", paddingTop: "1rem", marginBottom: "2rem", display: "flex", justifyContent: "space-between", fontSize: "1.1rem", fontWeight: "700" }}>
                <span>TOTAL</span>
                <span>₦{safeTotal.toLocaleString()}</span>
              </div>

              <button onClick={handleCheckout} disabled={loading} style={{ width: "100%", background: "#FF0000", color: "#fff", border: "none", padding: "1.25rem", fontSize: "0.85rem", fontWeight: "700", letterSpacing: "1.5px", cursor: loading ? "not-allowed" : "pointer" }}>
                {loading ? <ClipLoader size={20} color="#fff" /> : "CHECKOUT"}
              </button>
              
              <Link to="/shop" style={{ display: "block", textAlign: "center", marginTop: "1rem", color: "#777", fontSize: "14px" }}>Continue Shopping</Link>
            </div>
          </div>
        )}
      </div>
